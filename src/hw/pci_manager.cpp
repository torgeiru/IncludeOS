...
namespace hw {
  ...
  void PCI_manager::init_devices(const uint8_t classcode)
  {
    ...
    for(const auto& [pci_addr, id, devclass] : devinfos_)
    {
      ...
      // Having a logic flow exception for a VirtioFS device
      // A hacky but necessary thing to do given that VirtioFS and
      // VirtioBLK are both treated as storage devices although they
      // have different functionality and it would be even more hacky
      // for VirtioFS class to inherit from the Block_device class.
      if (stored_dev.vendor_product() == driver_id(PCI::VENDOR_VIRTIO, 0x105a)) {
